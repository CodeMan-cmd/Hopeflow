...
Hopeflow は以下の言語のインターフェースに対応しています：
...
Hopeflow はショートドラマ制作のためのAIワークベンチです。「企画→脚本→絵コンテ→映像出力」の完全なクローズドループを構築し、ローカライズ・プログラマブル・持続的イテレーションが可能な制作フローをサポートします。
...
[![Hopeflow 12分でわかるAI動画クイックスタート](./videoCover.jpg)](https://www.bilibili.com/video/BV1oXD7BqEqJ)

**Hopeflow 12分でわかるAI動画クイックスタート**
...
以下のスクリーンショットと動画は、Hopeflow を使って制作したAIショートドラマデモです。全体で約2時間で完成し、脚本生成、絵コンテ制作、編集を含みます。
...
| :------: | :----------------------------------------------------------- | :--------------------- |
| Windows  | [Release](https://github.com/HBAI-Ltd/Hopeflow-app/releases) | 公式リリースインストーラ |
|  Linux   | [Release](https://github.com/HBAI-Ltd/Hopeflow-app/releases) | 公式リリースインストーラ |
|  macOS   | [Release](https://github.com/HBAI-Ltd/Hopeflow-app/releases) | 公式リリースインストーラ |
...
docker run -d -p <ローカルポート>:10588 -v <ローカルデータパス>:/app/data Hopeflow
...
>
> **[智星雲（AI Galaxy）](https://www.ai-galaxy.com/)** は **Hopeflow 公式認定の商用イメージプロバイダー**で、Hopeflow AI ショートドラマ制作用の完全なイメージを合法的に搭載・配布・商用サポートしています。**すぐに使える、手動デプロイ不要**です。
>
...
2. イメージ選択：`windows10LTSCwin10_Hopeflow` - インスタンスを作成。
...
#### 二、Hopeflowの設定、ComfyUIの起動段階のチュートリアル

1. まずAgentが呼び出すモデルを設定します：デスクトップのHopeflowを開く - モデルサービス - OpenAI標準インターフェース - APIキーとリクエストアドレスを入力。
   ...
   - チャージ完了後、「Key管理」に入り - 新規API管理、名前は`Hopeflow`など任意で構いません、確定してAPIキーをコピーします。
2. 手順1に戻り、生成されたAPIキーとリクエストアドレスをHopeflowのモデルサービスに貼り付け、入力後に空白部分をクリックすると、システムが「プロバイダー設定が更新されました」と表示します。
   「手動追加」をクリックし、智星雲 - Tokenマーケットページに戻り、完全なモデル名をコピーします。
   > 💡 1つのKeyで智星雲上のすべてのモデルを呼び出せます。使いたいものを選んでください。`deepseek-v4-pro`がおすすめです。
   モデル名を完全にコピーしてHopeflowに貼り付け、確認してモデル設定を完了します。
...
>
> 開発者の皆様のHopeflowへの共同参加を歓迎します。参加ご希望の方は、メールで主催者ACTまでご連絡ください。
...
3. **開発環境の起動**
   ...
   - **方法3：本番モードでの起動**
     ...
     > 本番モードでコンパイル済みのサービスを直接実行します（事前に `pnpm build` が必要です）。
...
| ---------------- | ---------------------------------- | -------------------------------------------------- | ------------------------------------------------ |
| **Hopeflow-app** | 完全なクライアント（本リポジトリ、一般ユーザー向け） | [GitHub](https://github.com/HBAI-Ltd/Hopeflow-app) | [Gitee](https://gitee.com/HBAI-Ltd/Hopeflow-app) |
| **Hopeflow-web** | フロントエンドソース（フロントエンド開発者向け）       | [GitHub](https://github.com/HBAI-Ltd/Hopeflow-web) | [Gitee](https://gitee.com/HBAI-Ltd/Hopeflow-web) |

> 💡 **ヒント**：Hopeflow をただ使いたいだけの場合は、本リポジトリのクライアントをダウンロードしてください。フロントエンドリポジトリは二次開発やフロントエンドのカスタマイズを行う開発者向けです。

---

# 💌 お問い合わせ

📧 メール：[ltlctools@outlook.com](mailto:ltlctools@outlook.com?subject=Hopeflow問い合わせ)

---

# 📜 ライセンス

Hopeflow は Apache-2.0 ライセンスのもとでオープンソース公開されており、補足的な商用利用規約が付帯しています。
...
- Hopeflow の識別表示や著作権情報を削除または変更してはなりません。

## 永久無料のケース

- ✅ Hopeflow を使ってコンテンツを制作し、プラットフォームから収益を得る
- ✅ 二次開発して自分のチーム内で使用する
- ✅ 5法人以下が共同運営で内部利用する
- ✅ 個人の学習、研究、非商用利用
...
以下のオープンソースプロジェクトが Hopeflow に強力なサポートを提供しています：
...
以下の組織・団体・個人が Hopeflow をサポートしています：
