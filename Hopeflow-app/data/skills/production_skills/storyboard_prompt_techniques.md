...
| 分镜表字段 | 提示词中须体现 | 校验要点 |
|-----------|---------------|--------|
| 画面描述（含角色动作/朝向/空间关系/情绪） | 提示词正文【画面】段的核心内容 | 所有视觉主体、空间关系、动作、朝向、关键细节是否**零遗漏**保留 |
| 场景（场头「场景名」） | 提示词正文【画面】段的环境锚定 | 场景类型是否一致 |
| 景别 | 景别构图词 | 景别是否匹配（复合景别取首帧起始端） |
| 台词 | 不进入提示词 | 不输出 |
| 音效 | 不进入提示词 | 不输出 |
| 关联资产名称/ID | 图像资产标注 `@图N` | 是否逐一绑定、无遗漏无多余 |

> 分镜表无「角色动作/朝向/空间关系/情绪/光影氛围」独立列——动作、朝向、空间关系、情绪均承载于「画面描述」列。提示词不输出光影描述（光影由场景资产图承担，见「提示词结构框架」）。
...
| 分镜字段 | 提示词对应处理 |
|----------|----------------|
| 画面描述 | **主干内容**：提示词正文【画面】段的核心信息来源。须完整保留画面描述中的**所有**可见主体、空间层次、动作、朝向、关键细节、镜头关系，仅将叙事语言转换为视觉描述格式。严禁删减关键元素、替换为不同语义或自行添加画面描述中不存在的视觉元素 |
| 场景 | 取自分镜表**场头**「场景名」，融入【画面】段作为环境锚定，叠加风格专属技法的场景质感约束词 |
| 景别 | 镜头构图词（见下方景别词库），须与分镜表「景别」字段匹配。复合景别（如"远景→中景"）取**首帧起始端** |
| 运镜 | 仅作分镜制作信息，不进入提示词，不输出运镜备注 |
| 角色动作 | 分镜表无独立列，从「画面描述」中提取动作与朝向，按"首帧识别原则"处理。必须保留动作语义内涵与朝向标注 |
| 情绪 | 分镜表无独立列，从「画面描述」推断情绪基调，再从风格专属技法的情绪映射表中选取匹配的面容/眼神词 |
| 光影氛围 | 分镜表无此列（分镜表禁光影色调词）。**不生成【光影】段、不输出任何光影描述**——光影由场景资产图承担 |
| 台词 | 不进入提示词，不输出 |
| 音效 | 不进入提示词，不输出 |
...
提示词正文采用**两段式结构**（【画面】+【风格】），确保画面描述占据主干地位：

```
【画面】→ 承载分镜表「画面描述」（含角色动作/朝向/空间关系/情绪）+ 场头「场景名」+「景别」的完整视觉内容（主干，信息密度最高）
【风格】→ 风格锚定词 + 画质锁定词 + 禁止项声明（辅助修饰，简短）
```

> **篇幅分配原则**：【画面】段为信息密度最高、篇幅最长的段落，须完整承载分镜表「画面描述」的所有视觉元素；【风格】段最短，仅放必要的风格锚定词与画质锁定词。两段顺序不可颠倒、长度不可倒置——若风格词篇幅超过画面段，即为失败产出。
> **不生成【光影】段**：分镜表无「光影氛围」列且明确禁光影色调词，故提示词不输出任何光影/色温/明暗描述——光影由场景资产图参考自动承担（与分镜面板写入技能的全模式共享硬约束一致）。
...
【画面】{场头场景锚定}，{景别构图词}，{画面描述完整转写——保留所有视觉元素、空间关系、主体动作、朝向、情绪}。
...
> 分镜表无「光影氛围」列，本结构不包含【光影】段（光影由场景资产图承担）；【画面】段须完整承载「画面描述」列全部信息。

**关键规则**：
- 【画面】段须完整承载分镜表「画面描述」字段的所有信息，**不得删减**
- 【画面】段中，角色/场景/道具名称**必须使用 `@图N` 替代**（不用文字名称）
- 朝向信息须显式写入【画面】段（如"3/4正面朝右"）
- 不再追加英文段"Based on the reference image... Generate a new scene..."（`@图N` 机制已承担参考图绑定功能，追加英文段会导致画面描述出现两份、容易冲突）。⚠️ **历史残留**：早期风格技法示例中的该英文段已废弃，生成时一律不得输出
...
```json
{
  ...
  "shot": {
    "scene_and_framing": "[场头场景锚定 + 景别构图词]",
    "subject_and_action": "[画面描述完整转写：主体动作 + 朝向 + 情绪 + 所有视觉元素，使用@图N替代角色/场景名]",
    "style": "[风格锚定词 + 画质锁定词]"
  },
...
**关键规则**：
- `shot` 字段承载画面描述与风格词（分镜表无「光影氛围」列，故不含 `lighting` 字段——光影由场景资产图承担）
- `subject_and_action` 是信息密度最高的字段，须完整承载分镜表「画面描述」的视觉内容（含动作/朝向/情绪）
- 参考图作为图片输入，不是 URL 文本
...
分镜表无「角色动作/朝向」独立列，朝向信息承载于「画面描述」列的显式方位词中（如"面朝右""背对镜头""望向窗外"），提示词生成时**优先从画面描述直接提取**，并在 prompt 中**显式写入**对应朝向方位词（如 `facing right` / `面朝右`、`three-quarter view facing left` / `3/4侧面朝左`）。

**获取优先级**（高→低）：

| 优先级 | 线索来源 | 处理逻辑 |
|--------|---------|----------|
| **1** | **画面描述中的显式方位词** | 画面描述直接提及朝向（如"面朝右""背对镜头""望向窗外""面朝观众"）→ **直接采用**，无需推断 |
| 2 | **多角色空间关系（180° 视轴线）** | 对话/对峙/互动场景中，两角色面朝彼此：画面左侧角色面朝右，画面右侧角色面朝左。首次出场建立基准后全场景锁定 |
| 3 | **景别暗示** | 过肩镜：前景人物背对/侧背对镜头，远景人物面朝镜头方向；特写/近景独白：默认 3/4 侧面 |
| 4 | **情绪与叙事语义** | 孤独/沉思/回忆 → 侧面轮廓或3/4背面；对抗/质问 → 正面或3/4正面朝向对方；躲避/羞涩 → 微侧头避开对方 |
| 5 | **场景空间逻辑** | 门口迎客 → 面朝门外；眺望风景 → 面朝风景方向；伏案书写 → 面朝桌面低头 |

> 优先级2~5仅作为画面描述未标注朝向时的兜底推断。

**获取步骤**：
1. 读取分镜表当前行「画面描述」中的显式方位词
2. 若标注存在且完整 → 直接采用，跳过后续优先级
3. 若缺失（如空镜行）→ 按优先级2~5逐条推断
...
- **朝向变化须有动作衔接**：角色朝向确需变化时（如转头、回身），当前分镜的 prompt 中须包含转向动作描写（如"微微转头朝向画面左侧"），且该转向须与分镜表「画面描述」一致，不可凭空改向
...
| 字段 | 内容 |
|------|------|
| 场头 | `## 场1：某场景出口 ｜ 参演角色：角色甲`（场景名取自场头） |
| 画面描述 | 开场黑场淡入，某场景出口大远景，人流涌动，醒目指示物立于画面右侧，角色甲背着道具X 独行人流中，镜头缓推至中景，他手攥道具Y 忽然停步仰望指示物，眼神紧张而笃定，3/4正面朝右 |
| 景别 | 远景→中景 |
| 台词 | 无台词 |
| 音效 | 环境音：人流嘈杂声；音效：指示物轻微嗡鸣 |
