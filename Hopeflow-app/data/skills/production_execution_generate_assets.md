...
| 生成资产图片 | `generate_deriveAsset({ ids: [衍生资产id列表] })` |

### 执行流程

1. 获取 `assets`，从各资产的 `derive` 中收集所有需要生成图片的衍生资产 id（仅收集新增后仍未生成图片的衍生资产）
2. 调用 `generate_deriveAsset({ ids: [衍生资产id列表] })` 生成图片（异步，发起即返回）

### 约束

- 前置条件：衍生资产分析已完成并写入
- 仅对有衍生状态且尚未生成图片的衍生资产发起生成
