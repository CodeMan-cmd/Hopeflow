...
export default (nsp: Namespace) => {
  nsp.on("connection", async (socket: Socket) => {
    ...
    // 全自动模式配置（临时，随连接销毁）：开启后决策层自动连续执行流水线
    let autoConfig: { auto: boolean; maxRetries: number } = { auto: false, maxRetries: 2 };
    ...
    socket.on("updateAutoConfig", (data: { auto: boolean; maxRetries?: number }) => {
      autoConfig.auto = data.auto;
      if (typeof data.maxRetries === "number" && data.maxRetries > 0) autoConfig.maxRetries = data.maxRetries;
      console.log("[productionAgent] 更新全自动模式配置:", autoConfig);
    });
    ...
    socket.on("disconnect", (reason) => {
      console.log("[productionAgent] 连接断开:", socket.id, reason);
      abortController?.abort();
      abortController = null;
      // 全自动模式为临时状态，随连接销毁重置
      autoConfig = { auto: false, maxRetries: 2 };
    });
  });
