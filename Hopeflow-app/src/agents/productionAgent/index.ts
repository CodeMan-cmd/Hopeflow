...
import { getKnowledgeContext } from "@/utils/agent/knowledgeRag";
...
export interface AgentContext {
  ...
  /** 全自动模式配置：开启后决策层自动连续执行流水线，不逐阶段等待用户确认 */
  autoConfig?: {
    auto: boolean;
    maxRetries: number;
  };
...
export async function runDecisionAI(ctx: AgentContext) {
  ...
  // 全自动模式：向决策层注入运行模式标记，提示词按"全自动模式"章节自动连续执行
  const autoPrompt = ctx.autoConfig?.auto
    ? `\n\n当前运行模式：全自动模式（自动重试上限：${ctx.autoConfig.maxRetries} 次）。请严格按"全自动模式"章节执行，不得向用户逐阶段确认。`
    : "";
  ...
  // 项目知识库语义检索注入（无数据时返回空串，不注入）
  const knowledge = await getKnowledgeContext(ctx.resTool.data.projectId, text, 5);

  const { fullStream } = await u.Ai.Text("productionAgent:decisionAgent", ctx.thinkConfig.think, ctx.thinkConfig.thinlLevel).stream({
    system: prompt + autoPrompt,
    messages: [
    ...
    tools: {
      ...memory.getTools(),
      // 全自动模式下禁用 present_choices，强制 Agent 自主确认、不向用户弹按钮
      ...useTools({ resTool: ctx.resTool, msg: ctx.msg, disablePresentChoices: ctx.autoConfig?.auto }),
...
async function createSubAgent(parentCtx: AgentContext) {
  ...
  // 项目知识库语义检索注入（所有子 Agent 共享同一检索结果，无数据时不注入）
  const knowledge = await getKnowledgeContext(resTool.data.projectId, parentCtx.text, 5);
...
async function createArtSkills(artName: string, storyName: string) {
  ...
  if (!isSkillDisabled(`art_skills/${artName}`)) skillList.push(...(await scanSkills(artWorkerPath + "/*.md")));
  if (!isSkillDisabled(`story_skills/${storyName}`)) skillList.push(...(await scanSkills(storyWorkerPath + "/*.md")));
  const mainSkills: { path: string; name: string; description: string }[] = [];
...
async function consumeFullStream(
  ...
  let thinkTime = 0;
  let fullResponse = "";
  // 收集工具执行失败信息，拼入返回文本，让上层（决策层）能感知执行失败并自动重试
  ...
  try {
    for await (const chunk of fullStream) {
      ...
      if (chunk.type === "reasoning-start") {
        ...
        // 工具执行失败（如 socket 写入失败），记录错误供上层感知
    ...
    // 存在工具失败时把错误追加到回复文本，使决策层能识别"执行层返回错误"并按重试规则重新派发
...
async function useProductionSkills(artName: string, storyName: string) {
  ...
  // 过滤已停用的画风/题材技能包与生产技能文件
  const skillList: string[] = [];
  if (!isSkillDisabled(`art_skills/${artName}`)) skillList.push(...(await scanSkills(artWorkerPath + "/*.md")));
  if (!isSkillDisabled(`story_skills/${storyName}`)) skillList.push(...(await scanSkills(storyWorkerPath + "/*.md")));
  for (const filePath of await scanSkills(productionPath + "/*.md")) {
    if (!isSkillDisabled(`production_skills/${path.basename(filePath)}`)) skillList.push(filePath);
  }
  const mainSkills: { path: string; name: string; description: string }[] = [];
  for (const skillPath of skillList) {
    if (!fs.existsSync(skillPath)) throw new Error(`主技能文件不存在: ${skillPath}`);
