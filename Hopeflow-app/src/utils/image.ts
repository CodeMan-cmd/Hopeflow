 ...
 */
// 进程内并发去重锁：key=目标路径
const inflightThumbnail = new Map<string, Promise<string | null>>();
...
export async function ensureThumbnail(
  ...
  // 并发去重：同一缩略图只生成一次
  const inflight = inflightThumbnail.get(thumbnailPath);
  if (inflight) return inflight;
  const task = generateThumbnail(originalPath, thumbnailPath, size).finally(() => {
    inflightThumbnail.delete(thumbnailPath);
  });
  inflightThumbnail.set(thumbnailPath, task);
  return task;
}
