...
/**
 * 结果文件下载失败错误。
 * 当服务端（无代理）无法从供应商 CDN 下载结果文件时抛出，
 * 由上层路由捕获后将任务置为"待下载"，交由浏览器（走用户代理）下载并回传保存。
 */
export class DownloadNeededError extends Error {
  /** 结果文件的原始 URL，供浏览器端下载 */
  url: string;
  constructor(url: string) {
    super(`结果下载失败，请点击"浏览器下载"完成获取`);
    this.name = "DownloadNeededError";
    this.url = url;
  }
}
...
// 读取某供应商已启用的敏感词替换规则（每供应商独立配置，存于 o_vendorSafetyReplacements）
async function loadSafetyReplacements(vendorId: string): Promise<[string, string][]> {
  try {
    const rows = await u
      .db("o_vendorSafetyReplacements")
      .where({ vendorId, enabled: 1 })
      .orderBy("id", "asc")
      .select("replaceFrom", "replaceTo");
    return rows.map((r) => [String(r.replaceFrom), String(r.replaceTo)]);
  } catch {
    return []; // 表尚未初始化或查询异常时降级为不替换，不阻塞生图
  }
}

// 顺序应用替换（按规则配置顺序，触发词在前、替换词在后）
function applyReplacements(prompt: string, rules: [string, string][]): string {
  let result = prompt;
...
async function getVendorTemplateFn(fnName: FnName, modelName: `${string}:${string}`): Promise<any> {
  ...
  if (fnName == "textRequest")
    ...
    return async <T>(input: T) => {
      // 图片生成统一应用该供应商的敏感词替换规则（规避平台内容安全审核误判导致的 400）
      let finalInput = input;
      if (fnName === "imageRequest" && (input as any)?.prompt) {
        const replacements = await loadSafetyReplacements(id);
        if (replacements.length > 0) {
          finalInput = { ...(input as any), prompt: applyReplacements((input as any).prompt, replacements) };
        }
      }
      return fn(finalInput, selectedModel);
    };
}
...
async function withTaskRecord<T>(
  ...
  try {
    ...
    taskRecord(1, undefined, result);
    return result;
  } catch (e) {
    taskRecord(-1, u.error(e).message);
    // 下载失败类错误需原样透传，供上层路由识别为"待下载"状态
    if (e instanceof DownloadNeededError) throw e;
...
async function urlToBase64(url: string, retries = 3, delay = 1000): Promise<string> {
  for (let attempt = 1; attempt <= retries; attempt++) {
    // 单次尝试带超时，避免 CDN 限速/挂起导致任务长时间卡在"生成中"
    ...
    try {
      ...
      if (attempt === retries) {
        // 服务端下载失败，降级为"待下载"，由浏览器端下载
        throw new DownloadNeededError(url);
      }
...
class AiAudio {
  ...
  // input 透传给供应商 ttsRequest（TTSConfig 结构：text/voice/speechRate/pitchRate/volume/referenceList）
