...
// 向量搜索候选池上限：按时间取最近 N 条做比对，避免全表 O(n) 全扫描
const RAG_CANDIDATE_LIMIT = 500;
...
class Memory {
  ...
  async get(text: string) {
    ...
    // rag: 向量搜索 messages（候选池限最近 N 条，避免全表扫描）
    const queryEmbedding = await getEmbedding(text);
    const allMessages = await u
      .db("memories")
  ...
  async deepRetrieve(keyword: string) {
    ...
    const isolationKey = this.isolationKey;
    // 步骤1: 向量搜索 summary（候选池限最近 N 条）
    const queryEmbedding = await getEmbedding(keyword);
    const allSummaries = await u
      .db("memories")
