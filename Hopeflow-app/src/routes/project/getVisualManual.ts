...
const DATA_MAP: { label: string; value: string; subDir?: string }[] = [
  ...
  { label: "光影与氛围", value: "art_lighting_atmosphere", subDir: "art_prompt" },
  { label: "构图与镜头", value: "art_composition_camera", subDir: "art_prompt" },
  { label: "材质与质感", value: "art_texture_material", subDir: "art_prompt" },
...
export default router.post("/", async (req, res) => {
  try {
    ...
    const styleDirs = fs
      ...
      .filter((d) => d.isDirectory() && !isSkillDisabled(`art_skills/${d.name}`))
      .map((d) => d.name);
