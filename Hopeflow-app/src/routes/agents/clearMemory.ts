...
export default router.post(
  ...
  validateFields({
    ...
    agentType: z.enum(["scriptAgent", "productionAgent", "projectAgent"]),
    type: z.enum(["message", "summary", "all"]).optional(),
    sessionId: z.string().optional(),
  }),
  async (req, res) => {
    const { projectId, episodesId, agentType, type = "all", sessionId } = req.body;
    const isolationKey = `${projectId}:${agentType}${episodesId ? `:${episodesId}` : ""}${sessionId ? `:${sessionId}` : ""}`;
