...
export default router.post(
  ...
  async (req, res) => {
    ...
    const systemPrompt = visualManualDetails ? `${visualManual}\n\n---\n\n${visualManualDetails}` : visualManual;
    try {
      ...
      **${config.nameLabel}设定：**
      ...
      await u
        .db("o_assets")
        .where("id", assetsId)
        .update({ promptState: "失败", promptErrorReason: reason, errorType: classifyError(reason, { prompt: name }).type });
