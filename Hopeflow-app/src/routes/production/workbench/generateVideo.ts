...
import { ReferenceList, DownloadNeededError } from "@/utils/ai";
...
export default router.post(
  ...
  async (req, res) => {
    ...
    aiVideo
      ...
      .then(async () => await u.db("o_video").where("id", videoId).update({ state: "已完成" }))
      .catch(async (error: any) => {
        // 服务端下载受限：保存结果 URL，置为"待下载"，由前端引导浏览器下载后回传保存
        if (error instanceof DownloadNeededError) {
          await u
            .db("o_video")
            .where("id", videoId)
            .update({
              state: "待下载",
              errorReason: error.url,
              filePath: videoPath,
            });
          return;
        }
