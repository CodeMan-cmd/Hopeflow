...
export default router.post(
  ...
  async (req, res) => {
    ...
    const manualDetails = u.getVisualManualDetails(projectSettingData!.artStyle!);
    const appendDetails = (prompt: string) => (manualDetails ? `${prompt}\n\n---\n\n${manualDetails}` : prompt);
    const promptRecord: Record<string, { prompt: string }> = {
      role: {
        prompt: appendDetails(rolePrompt),
      },
      tool: {
        prompt: appendDetails(toolPrompt),
      },
      scene: {
        prompt: appendDetails(scenePrompt),
      },
    };
