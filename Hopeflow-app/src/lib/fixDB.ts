...
export default async (knex: Knex): Promise<void> => {
  ...
  // 统一 o_video 成功态状态词为"已完成"（历史数据曾写入"生成成功"）
  await db("o_video").where("state", "生成成功").update({
    state: "已完成",
  });

  // 失败原因分类字段（审核/尺寸/额度/未部署…），供界面按类别聚合与自愈
  await addColumn("o_assets", "errorType", "string");
  await addColumn("o_storyboard", "errorType", "string");

  // ---- 清理「旧错误不清空」的历史脏数据 ----
  // 生成成功之后错误栏还挂着上一次的报错，用户会以为还在坏。
  // 只清理已成功/进行中的行，失败行原样保留（那是真实错误）。
  await db("o_assets").where("promptState", "已完成").whereNot("promptErrorReason", null).update({
    promptErrorReason: null,
    errorType: null,
  });
  await db("o_storyboard").where("state", "已完成").whereNot("reason", null).update({
    reason: null,
    errorType: null,
  });
  // 失败行补一次分类，让界面能直接按「审核/尺寸/额度/未部署」聚合，不用逐个点开
  const untypedAssetFailures = await db("o_assets").whereNotNull("promptErrorReason").whereNull("errorType").select("id", "promptErrorReason");
  for (const row of untypedAssetFailures) {
    await db("o_assets")
      .where("id", row.id)
      .update({ errorType: classifyError(row.promptErrorReason).type });
  }
  const untypedStoryboardFailures = await db("o_storyboard").whereNotNull("reason").whereNull("errorType").select("id", "reason");
  for (const row of untypedStoryboardFailures) {
    await db("o_storyboard")
      .where("id", row.id)
      .update({ errorType: classifyError(row.reason).type });
  }

  // 添加新字段
  await addColumn("o_prompt", "useData", "text");
  // 任务返回参数（供应商请求成功后返回的数据）
  await addColumn("o_tasks", "result", "text");
  // 剧本排序字段
  await addColumn("o_script", "sort", "integer");
  // 剧本知识库提取状态字段（-1失败 0正在提取 1成功 2等待提取）
  await addColumn("o_script", "knowledgeState", "integer");
  await addColumn("o_script", "knowledgeErrorReason", "text");
  await db("o_script").where("knowledgeState", 0).update({
    knowledgeState: -1,
    knowledgeErrorReason: "软件退出导致失败",
  });
  // 项目是否启用改编策略（"1"启用 / "0"不启用）
  await addColumn("o_project", "needAdaptation", "string");
  // 添加新字段
  ...
  // 知识库设定向量（供生产 Agent 语义检索注入）
  await addColumn("o_knowledge", "embedding", "text");
  await addColumn("o_assets", "audioBindState", "integer");
  // audioBindState 实际写入字符串状态（生成中/已完成/生成失败），列类型由 integer 矫正为 string
  await alterColumnType("o_assets", "audioBindState", "string");
  await addColumn("o_modelPrompt", "fileName", "string");
  await addColumn("o_modelPrompt", "path", "string");
  // 启用 TTS 配音模型位（IndexTTS2 等 TTS 供应商模型可在此绑定）
  await u.db("o_agentDeploy").where("key", "ttsDubbing").update({ disabled: false });
  ...
  // 文生音色供应商（DashScope Voice Design）：承载 API Key 配置，供角色独有音色「文字描述生成」模式使用
  const dashscopeVendor = await u.db("o_vendorConfig").where("id", "dashscope").select("id").first();
  if (!dashscopeVendor) {
    await u.db("o_vendorConfig").insert({
      id: "dashscope",
      inputValues: "{}",
  ...
  //检测是否包含知识库设定提取提示词
  const existKnowledgePrompt = await db("o_prompt").where("type", "knowledgeExtraction").first();
  if (!existKnowledgePrompt)
    await db("o_prompt").insert({
      name: "知识库设定提取",
      type: "knowledgeExtraction",
      data: `# 知识库设定提取指令

你是剧本设定分析助手。用户会提供一集或多集剧本内容，你需要从中提取该作品的世界观、人物、术语、其他设定，形成结构化设定条目，供项目知识库长期使用。

## 输出要求

**必须通过调用 resultTool 工具返回结果**，禁止以纯文本、Markdown 表格或 JSON 代码块形式直接输出。resultTool 的 schema 如下：

\`\`\`
{
  items: [
    { type: "world" | "character" | "term" | "other", title: string, content: string }
  ]
}
\`\`\`
...
| type | 含义 | 提取内容 |
|------|------|----------|
| world | 世界观 | 世界背景、时代设定、力量体系、社会结构、地理环境、核心规则等 |
| character | 人物 | 有名字角色的设定：身份、性格、外貌、能力、人物关系等 |
| term | 术语 | 作品专属名词：专有技能、地名、组织、物品名等及其解释 |
| other | 其他 | 不属于以上三类但对创作重要的设定（如重要伏笔、专属风格基调） |

## 提取规则

1. **忠于原文**：只提取剧本中实际出现的内容，不推测、不脑补、不扩展原文未提及的信息
2. **title 简洁**：标题用名词短语，如"灵气体系""林逸""天枢阁"，不要带"设定/介绍"等后缀
3. **content 精炼**：30-200 字，客观陈述设定，多集信息可合并，去重
4. **人物归属**：每个主要角色独立一条（type=character），不要把多个角色挤进一条
5. **重要伏笔/关系**：可在 other 中单列（如"林逸与白有容的婚约"）
6. **控制数量**：单次提取 5-30 条，优先提取对后续创作（剧本、分镜、视频）有实际价值的设定
7. 多集剧本同时提供时，跨集合并相同设定，不重复输出`,
    });
  ...
  const advancedAgentList = [
    ...
    { key: "projectAgent:decisionAgent", name: "项目创建Agent:决策层", desc: "决策层" },
  ...
  //添加数据简易配置（主 Agent 记录，简易模式下按主 key 拆分配置）
  const ordinaryAgentList = [
    {
      key: "projectAgent",
      name: "项目创建Agent",
      desc: "与用户对话收集创作需求，生成项目信息与初始小说/剧本内容，建议使用文本理解与生成能力强的模型",
    },
  ];
  for (const agent of ordinaryAgentList) {
    const exists = await db("o_agentDeploy").where("key", agent.key).select("*").first();
    if (!exists) {
      await db("o_agentDeploy").insert({
        model: "",
  ...
  // 补充抽象视频画风记录（已有数据库升级用）
  const existingAbstract = await knex("o_artStyle").where("name", "abstract_meme").first();
  if (!existingAbstract) {
    await knex("o_artStyle").insert({
      id: 1001,
      name: "abstract_meme",
      label: "抽象视频",
      fileUrl: "",
      prompt: "物品拟人化·超现实·黑色幽默，适用于抖音爆款抽象视频（物品噩梦/物品说话/拟人短剧）",
    });
  }

  // 火山引擎Plan 已并入「火山引擎(豆包)」（volcengine 2.5 双通道），清理旧供应商记录，避免迁移逻辑将其写回
  await knex("o_vendorConfig").where("id", "volcenginePlan").delete();
  ...
  // 删除从未被写入的死列 selectVideoId（选中视频以 o_videoTrack.videoId 为准）
  await dropColumn("o_videoTrack", "selectVideoId");

  // 火山引擎(豆包)：2.5 起合并 Plan 套餐（按量付费 + Plan 付费双通道），低于 2.5 强制覆盖为最新代码
  ...
  const HopeflowVer = await u.vendor.getVendor("Hopeflow").version;
  if (Number(HopeflowVer) < 3.2) {
    u.vendor.writeCode("Hopeflow", vendorData["Hopeflow.ts"]);
  }
};
