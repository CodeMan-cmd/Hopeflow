      ...
      initData: async (knex) => {
        await knex("o_agentDeploy").insert([
          ...
          },
          {
            ...
            modelName: "",
            vendorId: null,
            key: "projectAgent:decisionAgent",
            name: "项目创建Agent:决策层",
            desc: "决策层",
            temperature: 1,
            maxOutputTokens: 0,
...
7. 多集剧本同时提供时，跨集合并相同设定，不重复输出`,
    ...
    //剧本
    {
      ...
      builder: (table) => {
        ...
        table.integer("knowledgeState");
        table.text("knowledgeErrorReason");
        table.integer("createTime");
        table.text("errorReason");
        table.integer("sort");
        table.primary(["id"]);
    ...
    //资产表
    {
      ...
      builder: (table) => {
        ...
        table.string("audioBindState");
        table.text("promptErrorReason");
        table.string("errorType"); // 失败原因分类（审核/尺寸/额度/未部署…），见 utils/errorClassify.ts
        table.primary(["id"]);
        table.unique(["id"]);
      },
      initData: async (knex) => {},
    ...
    // 成片（视频导出后回传落盘的最终产物，与分镜视频 o_video 隔离）
    {
      name: "o_output",
      builder: (table) => {
        table.integer("id").notNullable();
    ...
    //供应商敏感词替换规则表（每个供应商独立一套「触发词→替换词」规则，用于规避平台内容安全审核误判）
    {
      name: "o_vendorSafetyReplacements",
      builder: (table) => {
        table.increments("id");
        table.string("vendorId").notNullable(); //供应商id（对应 o_vendorConfig.id）
        table.text("replaceFrom").notNullable(); //触发词（需替换的原文）
        table.text("replaceTo").notNullable(); //替换词（安全措辞）
        table.integer("enabled").notNullable().defaultTo(1); //是否启用 1启用 0停用
        table.integer("createTime").notNullable();
        table.integer("updateTime").notNullable();
        table.unique(["vendorId", "replaceFrom"]);
      },
    },
    //AI 洞察报告历史表（Dashboard 生成的 AI 分析报告持久化保存，支持历史记录查看）
    {
      name: "o_aiInsightReport",
      builder: (table) => {
        table.increments("id");
        table.string("model").notNullable(); //生成报告所用的文本模型
        table.text("report").notNullable(); //Markdown 报告全文
        table.text("statsSnapshot"); //生成时的统计数据快照（JSON），便于还原上下文
        table.integer("createTime").notNullable(); //生成时间（毫秒时间戳）
      },
    },
  ];
  ...
  // 高频查询列索引（幂等：CREATE INDEX IF NOT EXISTS，兼容已有库，不重建表）
  const queryIndexes: Array<[string, string, string[]]> = [
    ["idx_script_projectId", "o_script", ["projectId"]],
    ["idx_assets_scriptId", "o_assets", ["scriptId"]],
    ["idx_assets_projectId", "o_assets", ["projectId"]],
    ["idx_assets_type", "o_assets", ["type"]],
    ["idx_image_assetsId", "o_image", ["assetsId"]],
    ["idx_image_state", "o_image", ["state"]],
    ["idx_storyboard_scriptId", "o_storyboard", ["scriptId"]],
    ["idx_storyboard_projectId", "o_storyboard", ["projectId"]],
    ["idx_storyboard_trackId", "o_storyboard", ["trackId"]],
    ["idx_storyboard_flowId", "o_storyboard", ["flowId"]],
    ["idx_video_videoTrackId", "o_video", ["videoTrackId"]],
    ["idx_video_projectId", "o_video", ["projectId"]],
    ["idx_video_scriptId", "o_video", ["scriptId"]],
    ["idx_videoTrack_projectId", "o_videoTrack", ["projectId"]],
    ["idx_videoTrack_scriptId", "o_videoTrack", ["scriptId"]],
    ["idx_videoTrack_state", "o_videoTrack", ["state"]],
    ["idx_output_projectId", "o_output", ["projectId"]],
    ["idx_output_scriptId", "o_output", ["scriptId"]],
    ["idx_tasks_projectId", "o_tasks", ["projectId"]],
    ["idx_tasks_taskClass", "o_tasks", ["taskClass"]],
    ["idx_novel_projectId", "o_novel", ["projectId"]],
    ["idx_eventChapter_eventId", "o_eventChapter", ["eventId"]],
    ["idx_eventChapter_novelId", "o_eventChapter", ["novelId"]],
    ["idx_agentWorkData_projectId", "o_agentWorkData", ["projectId"]],
  ];
  for (const [indexName, table, columns] of queryIndexes) {
    try {
      const cols = columns.map((c) => `"${c}"`).join(", ");
      await knex.raw(`CREATE INDEX IF NOT EXISTS "${indexName}" ON "${table}" (${cols})`);
    } catch (e) {
      console.warn(`[初始化数据库] 创建索引失败 ${indexName}:`, e);
    }
  }
};
