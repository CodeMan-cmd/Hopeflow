  ...
  <div class="memoryConfig">
    ...
    <t-form :data="formData" labelAlign="top" labelWidth="180px" class="memoryForm" @submit="handleSave">
      <t-card :title="$t('settings.memory.vectorModelConfig')" :bordered="true" style="margin-top: 16px">
        ...
        <t-form-item :label="$t('settings.memory.modelDevice')" name="modelDevice">
          <t-select v-model="formData.modelDevice">
            <t-option v-for="item in deviceOptions" :key="item.value" :value="item.value" :label="item.label" />
          </t-select>
          <template #help>{{ $t("settings.memory.modelDeviceHelp") }}</template>
        </t-form-item>
      </t-card>
...
// 推理设备：cpu 纯 CPU；dml 使用 DirectML（Windows GPU）；auto 自动优先 GPU，失败回退 CPU
const deviceOptions = [
  { value: "cpu", label: "CPU" },
