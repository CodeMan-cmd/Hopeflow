<template>
  <div class="about">
    <t-card bordered :style="{ width: '100%' }" class="logoCard">
      <div class="f">
        <img src="@/assets/logo.png" alt="Hopeflow Logo" class="logo" @click="onLogoClick" />
        <div class="appName">
          <div class="name">Hopeflow</div>
...
import HopeflowLogo from "@/assets/logo.svg";
...
const updateSource = ref<UpdateSource>("Hopeflow");
const updateSources = ref([
  {
    value: "Hopeflow" as UpdateSource,
    label: "Hopeflow",
    iconType: "image" as const,
    iconSrc: HopeflowLogo,
    iconClass: "Hopeflow",
    iconBg: "#ececec",
    disabled: false,
  },
...
async function doConfirmUpdate() {
  ...
  try {
    if (updateInfo.value.reinstall) {
      ...
      try {
        await fetch(`Hopeflow://openurlwithbrowser?url=${updateInfo.value.url}`);
      } catch (error) {}
      return;
    }
