<template>
  <div class="modelServe">
    ...
    <!-- TTS 模型测试弹窗 -->
    <TTSModelTest
      v-if="testingModel?.type === 'tts' && ttsTestVisible"
      v-model:modelVisible="ttsTestVisible"
      :vendorId="currentVendor!.id"
      :modelName="testingModel.modelName" />
      ...
      <div class="data">
        ...
        <div class="linkAdd" v-if="addMode == 'linkAdd'">
          <t-alert theme="warning" style="margin-bottom: 20px">
            请填写 TypeScript 代码文件的链接（.ts 文件），不要填 API 地址或其他无关链接。 确认后 Hopeflow 会自动加载该代码，请确保链接来源可信。
          </t-alert>
...
import TTSModelTest from "./vendorTest/TTSModelTest.vue";
...
const ttsTestVisible = ref(false);
