...
export type NodeType = NodeUploadData | NodePromptData | NodeNoteData | NodeGeneratedData;
...
export interface CleanNode {
  ...
  type: string;
  position: { x: number; y: number };
  data: UploadNodeData | PromptNodeData | NoteNodeData | Omit<GeneratedNodeData, "steps">;
}
...
// ===== 默认提示词节点数据 =====
export function createPromptData(prompt = ""): PromptNodeData {
  return {
    prompt,
  };
}
...
// ===== 默认备注节点数据 =====
export function createNoteData(text = ""): NoteNodeData {
  return {
    text,
  };
}
