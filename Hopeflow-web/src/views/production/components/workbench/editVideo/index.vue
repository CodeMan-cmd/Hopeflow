      ...
      <pane size="40" class="pr">
        <VideoTrack
          ...
          <template #custom-operation-import>
            <t-button variant="text" size="small" @click="handleImport" :title="$t('workbench.production.editVideo.importToMainTrack')">
              <template #icon><i-folder-open size="16" /></template>
              {{ $t("workbench.production.editVideo.importToMainTrack") }}
            </t-button>
          </template>
...
const clipConfigs = ref({
  video: {
    backgroundColor: "var(--app-gradient-accent)",
    borderColor: "#000000",
    height: 60,
    selected: {
      borderColor: "#ff6b6b",
...
async function handleExport() {
  ...
  try {
    const result = await videoPreviewRef.value.exportVideo();
    // 回传后端落盘保存为成片
    const projectId = project.value?.id;
    const scriptId = episodesId?.value ?? 0;
    ...
    await axios.post("/output/saveOutput", {
      projectId,
      scriptId,
      base64,
...
async function handleImport() {
  // 优先导入选中的视频，没有选中则按分镜分组取每个分镜的第一个视频
  let videosToImport = props.initialVideoItems.filter((item) => item.selected);

  if (videosToImport.length === 0) {
    const storyboardGroups = new Map<number, MediaItem[]>();
    for (const item of props.initialVideoItems) {
      const match = item.name.match(/#(\d+)-(\d+)/);
      if (match) {
        const storyboardNum = parseInt(match[1], 10);
        if (!storyboardGroups.has(storyboardNum)) {
          storyboardGroups.set(storyboardNum, []);
        }
        storyboardGroups.get(storyboardNum)!.push(item);
      }
    }
    videosToImport = [...storyboardGroups.entries()]
      .sort((a, b) => a[0] - b[0])
      .map(([, videos]) => videos[0])
      .filter(Boolean);
  }
  ...
  if (videosToImport.length === 0) {
    window.$message.warning($t("workbench.production.editVideo.noSelectedVideo"));
    return;
  }

  const mainTrack = tracksStore.mainTrack;
  if (!mainTrack) {
    window.$message.warning($t("workbench.production.editVideo.noMainTrack"));
    return;
  }

  // 清除主轨道上现有的片段
  const existingClipIds = mainTrack.clips.map((c) => c.id);
  if (existingClipIds.length > 0) {
    tracksStore.removeClips(existingClipIds);
  }

  // 按分镜顺序依次添加视频到主轨道
  let currentTime = 0;
  for (const video of videosToImport) {
    const duration = getDefaultDuration(video.type, video);
    const sourceUrl = video.url || video.id;
    const clipId = generateId("clip-");
    const clip: Partial<Clip> = {
      id: clipId,
      trackId: mainTrack.id,
      startTime: normalizeTime(currentTime),
      endTime: normalizeTime(currentTime + duration),
      type: "video",
      name: video.name,
      sourceUrl,
    ...
    if (!video.thumbnails || video.thumbnails.length === 0) {
      loadVideoClipThumbnails(tracksStore, clipId, sourceUrl);
    }

    currentTime += duration;
  }

  historyStore.pushSnapshot($t("workbench.production.editVideo.importToMainTrack"));
  window.$message.success(
    $t("workbench.production.editVideo.importSuccess", { count: videosToImport.length }),
  );
}
