...
// 抑制 av-cliper 日志（原在入口全局设置，随按需加载下移到唯一使用处）
...
// 导出视频：返回 blob 和元数据，由调用方决定保存方式
async function exportVideo(): Promise<{ blob: Blob; duration: number; width: number; height: number }> {
  if (!avCanvas) {
