  ...
  <div class="generatedNode">
    ...
    <div class="data" @click="selectedFn">
      ...
      <div class="image">
        ...
        <div v-else-if="generateError && !data.generatedImage" class="imageLoading generateErrorBox">
          <i-error size="28" class="generateErrorIcon" />
          <span class="loadingText generateErrorText">{{ generateError }}</span>
        </div>
...
async function handleGenerate() {
  ...
  try {
    ...
    // 失败原因展示在节点上，便于快速定位
    generateError.value = (e as any)?.message || $t("workbench.production.editImage.generateFailed");
    return window.$message.error(generateError.value);
  } finally {
