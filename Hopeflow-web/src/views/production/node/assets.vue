<template>
  <t-card class="assets">
    ...
    <div class="titleBar dragHandle">
      <div class="title">
        {{ $t("workbench.production.node.assets.title") }}
        <t-tag size="small" variant="light" style="margin-left: 6px">{{ assets.length }}</t-tag>
      </div>
    </div>
    <div class="content">
      <div class="cardGrid">
          ...
          <t-card class="assetCard">
            ...
            <div class="cardInfo">
              <div class="cardName">
                ...
                <!-- 提示词生成失败时把库里的原因抬到界面：以前只有「生成失败」四个字 -->
                <t-tooltip v-if="asset.promptState === '生成失败'" :content="promptFailReason(asset)">
                  <t-tag theme="danger" variant="light-outline">
                    {{ $t("workbench.production.node.assets.promptFailed") }}
                  </t-tag>
                </t-tooltip>
              </div>
 ...
 * 提示词失败原因的展示文案。
 *
 * 现场问题：界面上只有「提示词生成失败」四个字，真正的原因
 * （视觉手册未定义 / 敏感词 / 模型未部署）只写在 o_assets.promptErrorReason 里，
 * 得扒数据库才知道。这里把原因 + 分类一起显示。
 */
function promptFailReason(asset: AssetItem) {
  const type = asset.errorType || "unknown";
  const typeKey = `workbench.production.node.storyboard.errorType.${type}`;
  const typeLabel = $t(typeKey);
  const label = typeLabel === typeKey ? $t("workbench.production.node.storyboard.errorType.unknown") : typeLabel;
  const reason = asset.promptErrorReason || "";
  return reason ? `[${label}] ${reason}` : `[${label}] ${$t("workbench.production.node.assets.promptFailed")}`;
}
