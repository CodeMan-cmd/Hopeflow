  ...
  <div class="novel" ref="novelRef">
    <div class="headBtn jb ac" ref="headBtnRef">
      <t-space>
        ...
        <t-button variant="outline" @click="startEventAnalysis" :disabled="selectedRowKeys.length === 0">
          <template #icon>
    ...
    <t-table
      ...
      <template #empty>
        <AppEmpty :description="$t('workbench.novel.emptyDesc')" compact>
          <template #action>
            <t-button theme="primary" @click="importNovelFn">
              <template #icon>
                <t-icon name="add" />
...
async function pollEventState() {
  ...
  try {
    const { data } = await axios.post("/novel/getNovelEventState", { ids }, { silent: true, retry: true });
