...
async function handleBeforeUpload(file: UploadFile) {
  ...
  const allowTypes = ["text/plain", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "application/epub+zip"];
  const isEpub = rawFile.name.toLowerCase().endsWith(".epub");

  if (rawFile.type === "application/msword") {
    window.$message.warning($t("workbench.novel.import.msg.docNotSupported"));
    return false;
  }
  if (!allowTypes.includes(rawFile.type) && !isEpub) {
