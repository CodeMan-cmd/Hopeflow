...
export function useChat(options: UseChatOptions) {
  ...
  const setupHandlers = () => {
    ...
    socket.value.on("disconnect", (reason) => {
      ...
      // 断开时若仍在生成，将进行中的消息标记为 stop，避免界面永久停留在生成中
      if (status.value !== "idle" && currentMessageId.value) {
  ...
  const connect = () => {
    ...
    if (!socket.value) {
      socket.value = io(url, {
        ...
        timeout: 10000,
        path: "/api/socket.io",
        // 与服务端 maxHttpBufferSize 保持一致，防止大工作区数据（如分镜面板）超过默认 1MB 导致连接断开
  ...
  const reconnect = () => {
    // 销毁旧 socket 实例，确保重建连接时使用最新的连接配置（如 maxHttpBufferSize）
    if (socket.value) {
  ...
  const regenerate = (messageId: string) => {
    ...
    // 找到目标消息之前的最近一条用户消息，重新执行该指令（后端无 regenerate 事件，复用 chat 流程）
    ...
    for (let i = idx - 1; i >= 0; i--) {
      ...
      const text = (m.content || [])
        ...
        .join("\n")
        .trim();
      if (text) return emit("chat", { content: text });
      break;
    }
    return false;
  };
